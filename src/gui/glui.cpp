...
namespace glui
{
	...
	void renderTexture(gl2d::Renderer2D &renderer, glm::vec4 transform, gl2d::Texture t, gl2d::Color4f c, glm::vec4 textureCoordonates)
	{
		...
		renderer.renderRectangle((gl2d::Rect)newPos, t, c);
	}
		...
		float deltaTime)
	{
		...
		auto computePos = [&](int elementsHeight, float &advanceSizeY)
		{
			...
			{
				sizeWithPaddY = ((float)1 / elementsHeight);
			}

			float sizeY = sizeWithPaddY * inSizeY;
			float paddSizeY = sizeWithPaddY * (1 - inSizeY) / 2.f;

			float paddSizeX = (1 - inSizeX) / 2.f;

			glm::vec4 computedPos = {};
			computedPos.x = paddSizeX + (1 - mainInSizeX) * 0.5f;
			computedPos.y = paddSizeY + (1 - mainInSizeY) * 0.5f;
			computedPos.z = inSizeX * mainInSizeX;
			computedPos.w = inSizeX * mainInSizeY;
...

			{
				...
				switch (widget.type)
				{
					...
					case widgetType::textInput:
					{
						...
						std::string& inputString = *((std::string*)j.second.pointer);
						int pos = j.second.textSize;
						...
						std::string textCopy = inputString;
						...
						renderText(renderer, textCopy, font, transform, j.second.colors, true,
							!hovered);
	...
	void RendererUi::InputText(std::string name, std::string* text,
